...
public class SnakeAi {
	 ...
	 * @return
	 */
	public int play(Snake s, Node f) {// 这是一个完全弱智的走法，别用他
	...
	/**
	 * Make it right； BFS,不过当蛇去吃完后发现把自己围死就GG了
	 * 
	 * @param s 蛇
	...
	public int play1(Snake s, Node f) {
		...
		while (!q.isEmpty()) {
			...
			if (n.getX() == f.getX() && n.getY() == f.getY()) {
				...
				while (state != null && !state.equals(s.getFirst().toString())) {// 找到第一条路径
					// 食物去找蛇头
					stack.push(state);
			...
			// 判定下一步是否会撞到自己，如果找到出路则压入队列
			// 只要不撞到自己还有边界就全部收入到q vis path中
	...
	public int play2(Snake snake, Node f) {
		...
		if (realGoTofoodDir != -1) {
			...
			if (goToDailDir != -1)
				...
				snake.c++;
				/**
				 * 如果吃到后不能去自己尾巴，就跟着蛇尾跑 这里可能无限跟着蛇尾跑，主要原因是我追尾巴也用的BFS，追尾巴应该走最远距离
			...
			for (Node n : temp) {
				...
				if (!openList.contains(n)) {
					...
					if (n.equals(f)) {
						...
						// 从目标节点往前找，node不能用f，因为f与找到的节点虽然坐标相同但f没有记录father
